...
export function RiskMonitorPanel({
  ...
  return (
    <section className="panel-base mb-6 overflow-hidden p-5 sm:p-6" aria-live="polite">
      <div className="mb-6 flex items-start justify-between gap-3 border-b border-white/6 pb-4">
        <div>
          <h2 className="section-title">Risk Monitor</h2>
          <p className="mt-2 text-sm text-muted">
            Watch live risk signals, recent violations, and active alerts for the selected portfolio.
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-[0.72rem] font-semibold uppercase tracking-[0.2em] text-rose-300/85">
                <span className="inline-flex h-2.5 w-2.5 animate-pulse rounded-full bg-rose-300" />
                Critical risk alert
              </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <article className="rounded-2xl border border-white/6 bg-(--surface-container-low) px-4 py-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="typo-body font-semibold text-strong">Recent events</p>
            <span className="status-pill status-pill-neutral">{events.length} tracked</span>
          </div>

          {isLoading && <p className="typo-body-xs text-muted">Loading risk events...</p>}
          {!isLoading && error && <p className="typo-body-xs text-danger">{error}</p>}

          {!isLoading && !error && events.length === 0 && (
            <p className="typo-body-xs text-muted">No recent risk events for this portfolio.</p>
          )}

          {!isLoading && !error && events.length > 0 && (
            <div className="space-y-2">
              {events.slice(0, 4).map((event) => (
                <div key={event.id} className="rounded-xl border border-white/6 bg-(--surface-container) p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="typo-body-xs font-semibold text-strong">{eventMessage(event)}</p>
        ...
        <article className="rounded-2xl border border-white/6 bg-(--surface-container-low) px-4 py-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <span className="status-pill status-pill-neutral">
              {criticalAlerts.length > 0 ? `${criticalAlerts.length} critical` : `${activeAlerts.length} open`}
            </span>
          </div>
